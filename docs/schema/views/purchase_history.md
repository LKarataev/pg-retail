...
1.  **Определение транзакций клиента.** Для каждого клиента формируется
    ...
    [Транзакции](../tables/transactions.md). Связка с идентификатором клиента (`Customer_ID`
    таблицы [Персональные данные](../tables/personal_data.md)) осуществляется через
    идентификаторы всех карт клиента (поле `Customer_Card_ID` таблиц
    [Карты](../tables/cards.md) и [Транзакции](../tables/transactions.md)). Результат сохраняется
...
2.  **Определение дат совершения транзакций.** Для каждой транзакции
    ...
    [Транзакции](../tables/transactions.md), идентификация осуществляется по полю
    `Transaction_ID` таблиц История покупок и [Транзакции](../tables/transactions.md).
...
3.  **Определение списка SKU.** Для каждой транзакции указывается список
    ...
    `SKU_ID` таблицы [Чеки](../tables/checks.md). Сопоставление осуществляется на
    основании данных, содержащихся в полях `Transaction_ID` таблиц История покупок и [Чеки](../tables/checks.md).
...
5. **Определение списка групп.** Для каждого SKU на основании данных из
   ...
   матрица, сопоставление осуществляется по полю `SKU_ID` таблиц История покупок и [Товарная матрица](../tables/sku.md).
...
7.  **Подсчет финансовых показателей по группе.** Для каждого клиента по
    ...
    конкретную группу. Суммируются данные из таблицы [Чеки](../tables/checks.md).
    ...
    - Себестоимость купленного клиентом товара в течение 
      ...
      сохраняются в поле `Group_Cost` таблицы [История покупок](purchase_history.md).
    ...
    - Базовая розничная стоимость в течение анализируемого периода.
      ...
      `Group_Summ` таблицы [История покупок](purchase_history.md).
    ...
    - Фактически оплаченная стоимость (с учетом оплаты покупок
      ...
      анализируемой группы для клиента. Данные сохраняются в поле
      `Group_Summ_Paid` таблицы [История покупок](purchase_history.md).
