...
1.  **Формирование списков карт.** Для каждого клиента формируется
    ...
    `Customer_Card_ID` таблицы [Карты](../tables/cards.md).
...
2.  **Расчет среднего чека.** Для каждого клиента определяется величина
    ...
    [Транзакции](../tables/transactions.md). Суммируются все транзакции по всем картам каждого
    клиента по полю `Transaction_Summ` таблицы [Транзакции](../tables/transactions.md), после
...
5.  **Определение интенсивности транзакций.** Для каждого клиента
    ...
    `Transaction_DateTime` таблицы [Транзакции](../tables/transactions.md). Полученное значение
...
8.  **Определение периода после предыдущей транзакции.** Для каждого
    ...
    поздней на момент анализа транзакции. Для этого из [даты формирования анализа](../tables/date_of_analysis_formation.md) вычитается дата самой поздней транзакции
    клиента. Данные берутся из поля `Transaction_DateTime` таблицы [Транзакции](../tables/transactions.md) по всем картам клиента.
...
12. **Определение перечня магазинов клиента.** Для каждого клиента для
    ...
    содержащиеся в поле `Transaction_Store_ID` таблицы [Транзакции](../tables/transactions.md).
...
13. **Расчет доли транзакций в каждом магазине.** Для каждого магазина,
    ...
    таблицы [Транзакции](../tables/transactions.md).
